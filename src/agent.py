...
class Agent:
    ...
    def __init__(self, io_bus: IOBus, system_prompt: str = None, model: str = None,
    ...
    def _process_message(self, msg: IOMessage) -> str:
        ...
        # No steps — LLM answered directly (e.g. asked a clarifying question)
        if not plan["plan"]:
            self.history.append({"role": "user", "content": msg.text})
        ...
        # Step 3: Summarize — send results back to LLM for a final answer
        ...
        # Save the user message and final summary to history
        # (skip the intermediate plan/execution messages to keep history clean)
        self.history.append({"role": "user", "content": msg.text})
